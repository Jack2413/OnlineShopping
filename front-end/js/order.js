...
$(document).ready(function(e) {
  ...
  $("#cardbody").on("change", "#product_amount", function() {
    if (!$(this).val() == "") {
      ...
      var price_change =
        change_amount * parseFloat(product_price.replace(/[^0-9.-]+/g, ""));
...
function loadOrder() {
  $.ajax({
    method: "GET",
    url: url + "/getOrder/" + "Administrator@gmail.com"
...
function loadOrderDetails(data_Details) {
  ...
  data_Details.forEach(data_Detail => {
    ...
    if (permission == 0) {
      HTML +=
        '<div id="product-list" class="row" value=' +
        data_Detail.id +
        ">" +
        '<div class="col-12 col-sm-12 col-md-2 text-center">' +
        '<img class="img-responsive" src=../images/products/' +
        data_Detail.imagecode +
        '.png alt="prewiew" width="100" height="100">' +
        "</div>" +
        ...
        '<h6><strong id = "product_price">' +
        data_Detail.price +
        '<span class="text-muted">x</span></strong></h6>' +
        "</div>" +
        '<div class="col-4 col-sm-4 col-md-4" style="padding-top: 5px">' +
        '<div class="quantity">' +
        '<input id= "product_amount" type="number" step="1" max="99" min="1" value=' +
        data_Detail.amount +
        ' title="Qty" class="qty"size="4">' +
        "</div>" +
        "</div>" +
        '<div class="col-2 col-sm-2 col-md-2 text-right">' +
        '<button id="deleteButton" type="button" value = ' +
        data_Detail.id +
    ...
    total_price +=
      parseFloat(data_Detail.price.replace(/[^0-9.-]+/g, "")) *
      parseInt(data_Detail.amount);
  });
  ...
  if (permission == 0) {
    HTML +=
      ...
      '<div class="pull-right" style="margin: 5px">Total price: <b id = "total_price">' +
      total_price +
      "</b>" +
      "</div>" +
      "</div>" +
